...
**Current Phase:** WEEK 3/4 — Mistral-7B QLoRA, 8-bit inference, and 4-bit inference (all datasets) complete and confirmed on real hardware. Llama-2-13B's 8-bit and 4-bit inference (all datasets) are now **also complete** (2026-08-18/19) — both quantization techniques are done on both models. Llama-2-13B QLoRA is **deferred by choice, not blocked**, and its GPU-hour estimate is now considered **stale** (see GPU Budget Tracking) pending recalculation off real Llama-inference throughput. Mistral-7B ONNX export (the remaining Week 4 technique) is **deferred pending Ada cluster access** after four failed attempts on Kaggle's free tier — see Blockers & Risks "Deferred: Mistral-7B ONNX export." See EXPERIMENT_MATRIX.md's "Project-Level Finding: T4/Turing Quantization Slowdown — CONFIRMED" for the cross-technique, now cross-model pattern (QLoRA train, QLoRA inference, 8-bit inference, 4-bit inference — the latter two now confirmed on both Mistral-7B and Llama-2-13B — all substantially slower than baseline despite large VRAM savings).
...
- [ ] Llama-2-13B QLoRA remains separately deferred pending its own time-projection conversation — its ~6h estimate is now flagged **stale** (derived from the same parameter-count heuristic that just missed by ~4.5x on Llama inference) and needs recalculation calibrated off this batch's actual measured throughput before scheduling, not a straight reuse of the old number. Llama's ONNX export was never attempted and is deferred the same way as Mistral's (13B fp16 weights alone exceed 16GB VRAM, and its disk math is even worse than Mistral's) — see Blockers & Risks "Open Risk ... Llama-2-13B ONNX export"
