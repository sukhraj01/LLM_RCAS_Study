...
### Current Blockers
`EXP-MIS-ONNX-CNN`/`EXP-MIS-ONNX-SQUAD` have not yet run successfully — three attempts now (VRAM OOM, then disk OOM, then a failed `/opt/bin` redirect), see "Resolved (fix verified end-to-end against a stand-in model, not yet on Kaggle)" below. Not a GPU-budget blocker (fresh quota, see GPU Budget Tracking), but do not schedule the ONNX Kaggle session until the fix below is reviewed. Llama-2-13B QLoRA remains deferred by choice (Mistral 8-bit/4-bit was a faster, lower-risk use of the fresh quota), not blocked by an unresolved constraint.

### Resolved (fix verified end-to-end against a stand-in model, not yet on Kaggle): Mistral-7B ONNX export — three attempts, current fix avoids the disk math entirely (2026-08-15)
...
- [ ] Run Mistral-7B ONNX export/inference (`EXP-MIS-ONNX-CNN`/`EXP-MIS-ONNX-SQUAD`) — last Mistral-7B technique in the matrix. Three attempts on Kaggle so far (2026-08-15): VRAM OOM (fixed: CPU export, fp16) → disk-quota OOM (attempted fix: redirect to `/opt/bin`, which turned out read-only) → current fix loads the model in-process, frees its on-disk HF cache, then exports from the in-memory object via `optimum.exporters.onnx.onnx_export_from_model()`, so disk never needs to hold both the cache and the growing ONNX output at once. Verified end-to-end against a same-architecture (RMSNorm) stand-in model, **not yet reverified on the real 7B model on Kaggle** — see Blockers & Risks — rerun before marking this done
