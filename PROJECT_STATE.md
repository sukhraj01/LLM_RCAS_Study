...
**Current Phase:** WEEK 3/4 — Mistral-7B QLoRA, 8-bit inference, and 4-bit inference (all datasets) complete and confirmed on real hardware. Llama-2-13B's 8-bit and 4-bit inference (all datasets) are now **also complete** (2026-08-18/19) — both quantization techniques are done on both models. Llama-2-13B QLoRA is **deferred by choice, not blocked** — its stale ~6h estimate has now been **recalculated** (2026-08-19, projection only, no training run yet) using Mistral's measured LoRA→QLoRA ratios (~7.2-8.1x) stacked with the observed Llama/Mistral scaling factor (~1.8x, VRAM-grounded): CNN ~9.7-13.5h (risks the 12h session cap), SQuAD ~5.1-7.2h (fits with margin) — see EXPERIMENT_MATRIX.md's "Llama-2-13B QLoRA time projection" for full methodology and the explicit epistemic-gap caveat (no direct Llama training data exists yet). SQuAD is recommended as the calibration run; a real checkpointing gap was also found (200-step threshold unreachable at ~126 total steps) and flagged, not yet fixed. Mistral-7B ONNX export (the remaining Week 4 technique) is **deferred pending Ada cluster access** after four failed attempts on Kaggle's free tier — see Blockers & Risks "Deferred: Mistral-7B ONNX export." See EXPERIMENT_MATRIX.md's "Project-Level Finding: T4/Turing Quantization Slowdown — CONFIRMED" for the cross-technique, now cross-model pattern (QLoRA train, QLoRA inference, 8-bit inference, 4-bit inference — the latter two now confirmed on both Mistral-7B and Llama-2-13B — all substantially slower than baseline despite large VRAM savings).

**Project Status:** 🟢 On track feature-wise (Weeks 1-2 complete, Week 3 half-done on Mistral, Week 4 quantization complete on both models) and 🟡 GPU budget tightening — Llama's quantization-inference batch came in ~4.5x over its per-experiment estimate, pushing the project back over its planning-level budget (see GPU Budget Tracking below).
...
| **Experiment tracking** | ✅ In progress | 16/22 rows | `logs/experiment_tracking.csv` populated for all 4 Week 1 baselines + both Week 2 LoRA (Mistral) + both Week 3 QLoRA (Mistral) + both Week 4 8-bit (Mistral) + both Week 4 4-bit (Mistral) + both Week 4 8-bit (Llama) + both Week 4 4-bit (Llama) rows, all `CONFIRMED FINAL`, each now also tagged `hardware=T4`; 6 rows still `pending` (blank `hardware`, not yet run on anything) — Llama-2-13B QLoRA is deferred by choice with a now-**recalculated** (not yet run) GPU-hour projection (see GPU Budget Tracking / EXPERIMENT_MATRIX.md), Mistral + Llama ONNX (4 rows) deferred pending Ada cluster access (see Blockers & Risks) |
| **Week 2-4 experiments** | 🟡 In progress | 12/22 (Week 2 LoRA [Mistral only] + Week 3 QLoRA [Mistral only] + Week 4 8-bit/4-bit [both models, both datasets]) | Mistral LoRA, QLoRA, 8-bit, and 4-bit inference all complete and confirmed on real hardware. Llama-2-13B's 8-bit and 4-bit inference (both datasets) are now also complete (2026-08-18/19) — came in ~4.5x over their per-experiment GPU-hour estimate, see GPU Budget Tracking. Mistral ONNX (Week 4 remainder) is **deferred pending Ada cluster access**, not scheduled for further Kaggle attempts after 4 failures — see Blockers & Risks. Llama-2-13B QLoRA is deferred by choice, not abandoned, and its estimate is now recalculated (SQuAD ~5.1-7.2h, CNN ~9.7-13.5h, rough bound) but still not scheduled — engineer decision pending. |
...
### Open, pre-scheduling item (2026-08-19): Llama QLoRA checkpointing is not actually protective as configured
While confirming the "checkpoint every 200 steps" recovery procedure was wired in before Llama QLoRA could be scheduled (per `CLAUDE.md`), found that `TrainingArguments(save_steps=200, ...)` in `experiments/common.py` never actually fires for either Llama QLoRA dataset: `batch_size=1` × `gradient_accumulation_steps=16` = effective batch 16, and 1000 training examples × 2 epochs gives only ~126 total optimizer steps — below the 200-step threshold. The only save is the final `model.save_pretrained()` after training completes; a run killed mid-training (12h session cap, Kaggle preemption) has nothing to resume from. This is not new — Mistral's QLoRA runs had the identical effective-batch-16 math and the same gap, they just never hit an interruption. Not fixed yet (out of scope for this projection-only session) — recommend lowering `save_steps` (e.g. to ~25) before either Llama QLoRA experiment actually runs, especially `EXP-LLAMA-QLORA-CNN` given its projected range (~9.7-13.5h) risks the 12h cap. See EXPERIMENT_MATRIX.md's "Llama-2-13B QLoRA time projection" for the full writeup.
...
- [x] Llama-2-13B QLoRA's stale ~6h estimate **recalculated** (2026-08-19, projection only — see EXPERIMENT_MATRIX.md "Llama-2-13B QLoRA time projection"): CNN ~9.7-13.5h, SQuAD ~5.1-7.2h, explicitly a rough bound (stacks Mistral's measured LoRA→QLoRA ratio with an observed, not fully verified, cross-model scaling factor). SQuAD recommended as the calibration run (fits 12h cap; CNN's range risks it). Also found: `save_steps=200` checkpointing is unreachable at Llama QLoRA's ~126 total optimizer steps — flagged, not fixed.
- [ ] Llama-2-13B QLoRA training itself remains **not scheduled** — this was a projection/precondition-check exercise, not a go-ahead. Engineer decision pending on: (1) whether to schedule SQuAD as calibration, (2) whether to fix the `save_steps` gap first. Llama's ONNX export was never attempted and is deferred the same way as Mistral's (13B fp16 weights alone exceed 16GB VRAM, and its disk math is even worse than Mistral's) — see Blockers & Risks "Open Risk ... Llama-2-13B ONNX export"
