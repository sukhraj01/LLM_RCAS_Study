...
### Current Blockers
None. The GPU-budget hold from earlier today is resolved: Kaggle's weekly quota reset mid-QLoRA-session on 2026-08-15 (session was not interrupted, both experiments completed cleanly), giving a fresh 30h allocation — see GPU Budget Tracking "Kaggle Weekly Quota Reset" for the honest accounting. Llama-2-13B QLoRA remains deferred by choice (Mistral 8-bit/4-bit is a faster, lower-risk use of the fresh quota right now), not blocked by an unresolved constraint.
...
- [ ] Run Mistral-7B ONNX export/inference (`EXP-MIS-ONNX-CNN`/`EXP-MIS-ONNX-SQUAD`) — last Mistral-7B technique in the matrix
