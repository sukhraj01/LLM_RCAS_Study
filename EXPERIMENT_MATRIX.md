...
**Fix:** redirect both the HF model cache and the ONNX export output off the default output path before running an ONNX script: `%env HF_HOME=/opt/bin/hf_cache` and `%env ONNX_CACHE_DIR=/opt/bin/onnx_cache` (see `README.md` Kaggle setup). `experiments/mistral/06_onnx.py` and `experiments/llama/05_onnx.py` now read `ONNX_CACHE_DIR` (falling back to the previous `CHECKPOINTS_DIR`-based path when unset, so local/default behavior is unchanged) to compute `onnx_dir`. `HF_HOME` needs no code change — it's `transformers`/`huggingface_hub`'s own standard cache-location env var, already respected by every `from_pretrained()` call in this codebase. Checked whether `CHECKPOINTS_DIR` (used for LoRA/QLoRA adapter saves too) needed the same override: no — `run_training_experiment()`'s `model.save_pretrained(output_dir)` saves adapter-only weights (a few tens of MB for this project's LoRA/QLoRA configs), not full model weights, so it was never part of the ~20GB pressure; only the ONNX export path needed the override. Verified offline only (`py_compile`, full module import, manual trace of the `os.environ.get("ONNX_CACHE_DIR", CHECKPOINTS_DIR)` fallback with and without the env var set) — **not yet confirmed on a GPU**. Two consecutive ONNX failures on Mistral now (VRAM, then disk), both root-caused and fixed in code, neither yet confirmed end-to-end on real hardware.
