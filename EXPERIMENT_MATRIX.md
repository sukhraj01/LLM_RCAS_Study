...
| EXP-LLAMA-QLORA-CNN | Llama-2-13B | QLoRA | CNN | ~~3.0~~ 9.7-13.5 (proj.)\*\* | adapter, ROUGE |
...
**Total estimated: ~25.5 GPU hours (revised 2026-08-19; was ~21.3).** Budget 30-40 with reruns and mistakes (see `PROJECT_STATE.md` GPU Budget Tracking — 29.4h planned across weeks 1-4, buffer week reserved separately).
...
**\*\*Llama-2-13B QLoRA time projection — recalculated 2026-08-19, NOT YET RUN.** The old `3.0` GPU-hr/dataset figure was the same kind of round-number guess that missed by ~4.5x on Llama's 8-bit/4-bit inference batch above — it was not derived from any measured ratio. This projection replaces it with one built from two measured/observed ratios, and is explicit about what it does and doesn't actually know.
