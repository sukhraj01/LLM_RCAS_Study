...
*Checkpointing precondition — NOT currently protective as configured, flagged before either run starts:* `experiments/common.py`'s `run_training_experiment()` sets `TrainingArguments(save_steps=200, ...)` (default `save_strategy="steps"`), matching `CLAUDE.md`'s "checkpoint every 200 steps" recovery procedure on paper. But Llama's QLoRA hyperparameters (`batch_size=1`, `gradient_accumulation_steps=16` → effective batch 16) against 1000 training examples × 2 epochs give only **~126 total optimizer steps per dataset** (`⌈1000/16⌉ × 2`) — well under the 200-step threshold, so **the checkpoint would never actually fire during a full run of either dataset.** The only save that happens is the final `model.save_pretrained(output_dir)` after `trainer.train()` returns — if the run is killed mid-training (12h cap, Kaggle preemption, any crash), there is nothing to resume from. This is not new to Llama: Mistral's QLoRA runs (effective batch 2×8=16, same ~126-step math) had the identical gap and simply got lucky (no interruption). Given CNN's projected range now straddles the 12h cap, this gap is no longer theoretical for the run most likely to need it. **Recommend lowering `save_steps` (e.g. to 25, giving ~5 checkpoints across a ~126-step run) before scheduling either Llama QLoRA experiment** — not changed in this update, since this session's instruction was to project and confirm, not to modify training code.
