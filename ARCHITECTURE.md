...
## ADR-006: Week 3 QLoRA quality gate — drop the speed-comparison criterion
**Decision:** Revise the Week 3 (QLoRA) quality gate in `EXPERIMENT_MATRIX.md`. Old wording: "Both models' QLoRA experiments complete, peak VRAM stays under 14GB, training time visibly less than LoRA's." New wording: "Both models' QLoRA experiments complete, VRAM reduction confirmed vs each model's fp16 baseline, no NaN/divergence during training, quality metrics within documented bounds of baseline." The "training time visibly less than LoRA's" clause is dropped entirely, not loosened or reworded.

**Why:** This clause was written during initial project planning, before any QLoRA data existed — it encoded the popular assumption that QLoRA is a strictly faster/cheaper alternative to fp16 LoRA. This project's own measured data directly refutes that on T4/Turing hardware, not just falls short of it: Mistral's QLoRA trained ~14x *slower* per step than fp16 LoRA (183.97s/step vs 12.83s/step), and total training time was correspondingly longer (6.44h/3.42h vs 0.89h/0.42h) — the opposite direction from what the old gate expected. This is documented in `EXPERIMENT_MATRIX.md`'s "Project-Level Finding: T4/Turing Quantization Slowdown," a confirmed, six-data-point, two-model finding, not a one-off anomaly that might reverse on a rerun. For Llama-2-13B specifically, the old clause is additionally inapplicable on structural grounds, not just empirically wrong: Llama skips fp16 LoRA entirely (see ADR-002, doesn't fit 16GB), so there is no Llama LoRA training time to compare QLoRA against at all — the comparison the old gate demanded literally cannot be made for one of the two models it's supposed to cover.

**This is a correction to an invalidated premise, not a loosened standard.** The replacement criteria test what QLoRA is actually *for* on this hardware — enabling training within a tight VRAM budget without destabilizing training or degrading output quality — rather than a speed claim this project's own hardware findings have already disproven before the gate was ever checked. Revising the gate does not retroactively bless a bad result: re-evaluated against the new criteria, all four completed QLoRA training runs (Mistral CNN/SQuAD, Llama CNN/SQuAD) pass on their own merits — see `PROJECT_STATE.md`'s Quality Gate Evaluation for the row-by-row check.
