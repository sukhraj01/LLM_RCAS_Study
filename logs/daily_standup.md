...
### 2026-08-15 — Kaggle (ONNX export, Mistral-7B) — second OOM, disk this time, code fix applied, rerun still needed

**Planned:** Rerun `experiments/mistral/06_onnx.py` on Kaggle to confirm the CPU-export/fp16 VRAM fix from the previous entry.

**Completed:** No experiment rows produced this session either — the VRAM fix worked (export actually started running this time, no repeat of the earlier crash), but a second, different failure hit before export finished. Root-caused and fixed in code (see Issues); **not yet reverified on Kaggle hardware.**

**Issues:** Export died with Kaggle's disk-quota crash page: "Output: 20.94 GB." Confirmed this is not a repeat of session history bleeding over — this was a genuinely fresh Kaggle session — and not a real disk-space shortage — `df -h` showed 1.1TB free at the underlying overlay filesystem level. What's actually enforced is a separate, fixed **~20GB quota on the notebook's tracked *output*** (the default working directory the repo is cloned into, which is where `CHECKPOINTS_DIR` — and therefore the ONNX export — resolves to by default), independent of the raw disk. The math: the fp16 HF model cache (~14.5GB, under wherever `HF_HOME` defaults to) plus the ONNX export's own partial output (~6GB when it died, ~14GB if it had completed) together exceed the 20GB output quota on their own, regardless of whether the session has any prior history. `df -h` also surfaced two mounts with far more headroom that are presumed (not yet confirmed) to sit outside that output-quota tracking: `/opt/bin` (122GB, ~119GB free) and `/kaggle/lib` (20GB, nearly empty).

**Fix applied** (same session, local, no GPU used): both `experiments/mistral/06_onnx.py` and `experiments/llama/05_onnx.py` now compute `onnx_dir` from a new `ONNX_CACHE_DIR` environment variable, falling back to the previous `CHECKPOINTS_DIR`-based path when unset (local/default behavior unchanged). `README.md`'s Kaggle setup section gained a new step recommending `%env HF_HOME=/opt/bin/hf_cache` and `%env ONNX_CACHE_DIR=/opt/bin/onnx_cache` before running either ONNX script, with the quota explanation inline. Checked whether `CHECKPOINTS_DIR` itself needed the same treatment (it's also used for LoRA/QLoRA adapter saves) — confirmed no: `run_training_experiment()`'s `model.save_pretrained(output_dir)` saves PEFT adapter-only weights, a few tens of MB for this project's configs, never part of the ~20GB pressure — so the override was scoped to the ONNX path only rather than applied broadly. New Recovery Procedures entry added to `EXPERIMENT_MATRIX.md` ("ONNX Export Disk OOM," alongside the earlier VRAM one); `PROJECT_STATE.md` Blockers & Risks section rewritten to cover both consecutive failures under one entry.

**Verified offline only:** `py_compile` on both scripts, full module-level import against the real installed stack, and a manual trace of the `os.environ.get("ONNX_CACHE_DIR", CHECKPOINTS_DIR)` fallback confirming it resolves to the old default path when unset and to the redirected path when set. **No GPU or Kaggle disk environment available locally — cannot confirm this fix actually clears the quota until rerun on Kaggle.**

**GPU hours used this session:** 0.0 (died on the export/disk-write step before any GPU inference ran; fix itself was code-only, local, offline-verified)

**Next session:** Rerun `experiments/mistral/06_onnx.py` on Kaggle with `HF_HOME`/`ONNX_CACHE_DIR` redirected per the updated `README.md` — this is now the third attempt at this experiment and needs to clear both the VRAM fix (confirmed logically, not yet on hardware) and the disk fix (same) in the same run. If it clears both, confirm `peak_vram_gb` and quality are sane before marking Week 4's ONNX checklist item done. Still do not attempt Llama-2-13B's ONNX export — its VRAM open risk is unaffected by either of today's fixes.

---

### 2026-08-15 — Kaggle (ONNX export, Mistral-7B) — third failure (`/opt/bin` read-only), real fix: in-memory export

**Planned:** Rerun `experiments/mistral/06_onnx.py` on Kaggle with `HF_HOME`/`ONNX_CACHE_DIR` pointed at `/opt/bin`, per the previous session's disk-OOM fix.

**Completed:** No experiment rows produced this session again — the `/opt/bin` redirect itself failed before export could even start writing there. Root-caused and fixed for real this time (see Issues); the fix is now **verified end-to-end against a stand-in model**, not just offline-checked, but still not confirmed against the actual 7B model on Kaggle.

**Issues:** `/opt/bin` — recommended last session because `df -h` showed ~119GB free there — turned out to be **read-only** on Kaggle: `OSError: [Errno 30] Read-only file system`. Free space on a mount doesn't mean it's writable; should have flagged that as unconfirmed last session rather than recommending it outright. Reverted the README recommendation.

Re-examined the actual constraint instead of looking for another mount to redirect to: `main_export()` and `ORTModelForCausalLM.from_pretrained(export=True, ...)` both take a model *path/id string* and load the model internally as part of exporting — meaning the full fp16 HF cache (~14.5GB) and the growing ONNX output (~14GB) are structurally required on disk *at the same time*, regardless of which mount either lives on, as long as that mount has to hold both. No redirect fixes that if nothing writable has more than ~20GB.

Read `optimum.exporters.onnx.convert`'s source (same approach that found the fp32-default root cause two sessions ago) and found `onnx_export_from_model()` — the lower-level function `main_export()` itself calls into — which takes an **already-loaded PyTorch model object** rather than a path/id. Its own docstring shows exactly the pattern needed: load the model with `AutoModelForCausalLM.from_pretrained()` first, then export from that object directly.

**Fix applied** (same session, local, no GPU used): both `experiments/mistral/06_onnx.py` and `experiments/llama/05_onnx.py` now (1) load the model themselves via `AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.float16, token=HF_TOKEN)`, (2) free the on-disk HF cache for that model via `huggingface_hub.scan_cache_dir().delete_revisions(*commit_hashes).execute()` — the official cache-management API, which deletes the actual blob files, not just directory symlinks — and (3) export via `onnx_export_from_model(model=model, output=onnx_dir, task="text-generation-with-past", device="cpu")` against the in-memory object. Disk now only ever holds the cache *or* the growing ONNX output, never both simultaneously — Mistral's peak either way (~14.5GB) fits comfortably inside the ~20GB quota, so no mount redirect is needed at all anymore. `ONNX_CACHE_DIR` kept in the code (harmless, still useful if a genuinely writable larger mount ever turns up) but the README no longer recommends `/opt/bin` as its target.

**Verified safety of the cache deletion by reading transformers' real loading code, not by assuming it**: `transformers/modeling_utils.py`'s `load_state_dict()` loads each safetensors shard inside `with safe_open(checkpoint_file, framework="pt") as f: ... f.get_tensor(k)` — `get_tensor()` copies the tensor data into a fresh torch-owned buffer, and the `with` block closes the file/mmap before `from_pretrained()` returns. No file handle from the loading process survives to keep deleted disk blocks alive at the OS level, so freeing the cache after loading is genuinely safe, not just plausible.

**Verified this fix far more rigorously than the previous two attempts** — actually ran the complete pipeline end-to-end (not just `py_compile`/import) against `hf-internal-testing/tiny-random-MistralForCausalLM`, matching the real model's RMSNorm-based architecture family: loaded fp16 → freed the on-disk cache → confirmed the model still produced correct forward-pass output afterward (proving the cache deletion didn't corrupt anything) → exported via `onnx_export_from_model` → reloaded the exported graph via `ORTModelForCausalLM.from_pretrained(onnx_dir, provider=...)` → ran `generate()` and got coherent (if nonsensical, as expected from a tiny random-weight model) output. **All steps passed.** Also ran the identical pipeline against a tiny GPT2 model as a second check — this one failed at ONNX Runtime's internal graph-validation step with a `LayerNormalization` fp16/fp32 dtype-mismatch error, a known fp16-export quirk unrelated to this fix's logic. Confirmed this doesn't reproduce on the Mistral-architecture test (Mistral uses RMSNorm, no `LayerNormalization` op exists in its graph at all) — reported this finding plainly rather than hiding it, since it's a real caveat even though it isn't expected to affect the actual experiment.

**GPU hours used this session:** 0.0 (failed on a read-only filesystem error before any GPU time; fix itself was code-only, local, verified against a tiny stand-in model with no GPU needed)

**Next session:** Rerun `experiments/mistral/06_onnx.py` on Kaggle — no special env var setup needed this time, just the normal Kaggle setup cell. This is the fourth attempt at this experiment; if it clears cleanly, confirm `peak_vram_gb` and quality are sane before marking Week 4's ONNX checklist item done. Still do not attempt Llama-2-13B's ONNX export — its disk math (peak ~26GB either phase) and VRAM math (26GB fp16 weights vs 16GB ceiling) are both unresolved by this fix, tracked separately as an open risk.
